...
export function ExportView() {
  const { state, locale } = useWorkshop()
  const [copied, setCopied] = useState(false)
  const email = exportWorkshopEmail(state, locale)
    ...
    <div className="flex flex-col gap-5 max-w-3xl">
      <Surface>
        ...
        <p className="text-sm text-ink leading-relaxed">
          ...
          {state.outcomes.nextUpdate || REVIEW_DATE}.
        </p>
        <div className="flex gap-2 mt-4">
          <Button type="button" onClick={copy}>
            {copied ? tr(locale, "Copied ✓") : tr(locale, "Copy email")}
          </Button>
          <Button type="button" variant="outline" onClick={download}>
            {tr(locale, "Download .txt")}
          </Button>
        </div>
      </Surface>
...
function BoardRecord() {
  const { state, snapshots, restoreSnapshot, restoreFromJson, locale } = useWorkshop()
  ...
  const onFilePicked: React.ChangeEventHandler<HTMLInputElement> = async (
    ...
    try {
      ...
      if (!restoreFromJson(parsed)) {
        setRestoreError(tr(locale, "That file isn't a workshop board export."))
      }
    } catch {
      setRestoreError(tr(locale, "Couldn't read that file as JSON."))
    }
  }
  ...
  return (
    <Surface>
      <h2 className="heading-sm text-ink mb-1">{tr(locale, "Board record & backups")}</h2>
      ...
      <div className="flex flex-wrap gap-2 mb-2">
        <Button type="button" onClick={downloadBoard}>
          {tr(locale, "Download board file (.json)")}
        </Button>
        <Button
          type="button"
          variant="outline"
      ...
      <h3 className="label-md text-ink mt-4 mb-2">
        {tr(locale, "Snapshots on this browser")}
      </h3>
      {snapshots.length === 0 ? (
        <p className="text-sm text-muted">
          {tr(locale, "No snapshots yet — they appear automatically as you edit the board.")}
        </p>
      ) : (
            ...
            return (
              ...
              >
                <span className="text-sm text-ink tabular-nums">
                  {label}
                  <span className="text-muted"> · {tr(locale, snapshot.reason)}</span>
                </span>
                <Button
                  type="button"
                  size="sm"
